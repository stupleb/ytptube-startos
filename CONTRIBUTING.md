...
- **Authentication is on by default.** Upstream ships with none; this package generates a random admin password on install (`startos/init/watchAuth.ts`), injects `YTP_AUTH_USERNAME`/`YTP_AUTH_PASSWORD`, and exposes a `Reset Admin Password` action. The password lives in `store.json` on the `startos` volume. Since upstream 2.7.0 those env vars only seed the account while YTPTube's users table is empty, so the action rotates the password by running upstream's own `app.scripts.reset_password` against the database (`startos/actions/resetAdminPassword.ts`) rather than by rewriting the env.
...
- **File Browser download destination.** The `Select Download Destination` action can point downloads into [File Browser](https://github.com/Start9Labs/filebrowser-startos) instead of local storage. It subpath-mounts only the `ytptube-downloads` folder of File Browser's `data` volume at `/mnt/filebrowser/ytptube-downloads` (auto-created by the host on first mount; YTPTube cannot access the rest of the volume), and the setup oneshot `chown`s it to `app` + `chmod 755`s it. No `idmap` is needed on the mount: StartOS gives all services one shared on-disk id space, and both containers' users are uid 1000, so each side natively owns the files the other writes. It's an either/or replace — File Browser can't be a sub-folder alongside local downloads, because YTPTube's `calc_download_path` confines downloads to a single base path.
